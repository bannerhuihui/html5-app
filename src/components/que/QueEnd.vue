<template>
  <div class="container">
    ...
    <div class="content">
      ...
      <div class="lower-content-container" style="position: relative">
        <!-- 设置相对定位 -->
        <div class="lower-content">
          <div v-for="option in endQuestionList" :key="option.type">
            <van-button
              ...
              :class="{
                'button-que-button-active': selectedOption === option.type,
              }"
              :value="option.type"
              type="primary"
              block
              round
              @click="radioUpd(option.type)"
            >
              {{ option.content }}
            </van-button>
          </div>
    ...
    <!-- 底部区域 -->
    <div class="footer">
      <van-button class="end-button" @click="pushQuestion" round size="large"
        >完成</van-button
      >
    </div>
  </div>
