  ...
  <div>
    ...
    <div class="base-center">
      <van-row class="user-info">
        ...
        <van-col span="14">
          <div class="user-info-right" v-if="userInfo.riskTipsTags">
            {{ userInfo.riskTipsTags[0] }}
          </div>
        </van-col>
      </van-row>
      ...
      <van-row class="user-info">
        <van-col span="24">
          <ul class="user-info-left">
            <li>
              家族史：<span>{{ familyMedicalHistory }}</span>
            </li>
            <li>
              病历史：<span>{{ pastMedicalHistory }}</span>
            </li>
          </ul>
        </van-col>
      </van-row>
            ...
            <div class="base-item-cat">
              ...
              <ul class="base-item-cat-ul">
                ...
                <li v-if="diseases.description === '已病'">
                  <img
                    src="https://resource.cn-bj.ufileos.com/img/icon-success-temp.jpg"
                    alt=""
                  />
                </li>
                <li v-else>
      ...
      <div class="base-item">
        ...
        <div class="base-item-body">
          <div
            v-for="(indicatorAnaly, index1) in report.indicatorAnalysis"
            :key="index1"
          >
            <div class="like-progress">
              {{
                indicatorAnaly.indicatorName === "体质指数"
                  ? "BMI"
                  : indicatorAnaly.indicatorName === "收缩压"
                  ? "收缩压(高压)"
                  : indicatorAnaly.indicatorName === "舒张压"
                  ? "舒张压(低压)"
                  : indicatorAnaly.indicatorName
              }}&nbsp;&nbsp;{{ indicatorAnaly.indicatorValue }}
            </div>
      ...
      <div class="base-item">
        ...
        <div class="base-item-body">
          <div
            v-for="factor in report.riskFactorAnalysis"
            :key="factor.riskFactor"
          >
            <div class="fx-cat">
              <div class="fx-cat-titel">
                <h2>{{ factor.riskFactorName }}</h2>
              </div>
      ...
      <van-button class="base-button" @click="gotoNext" round size="large"
        >查看健康指导建议</van-button
      >
    </div>
  </div>
...
if (res) {
          ...
          this.pieData = [
            {
              value: parseInt(this.report.healthIndex, 10),
              name: "A",
              itemStyle: { color: "#51d2c8" },
            },
            {
              value: parseInt(100 - this.report.healthIndex, 10),
              name: "B",
              itemStyle: { color: "#FFF" },
            },
          ];
  ...
  methods: {
    gotoNext() {
      ...
      this.$router.push({
        path: "/next",
        query: { info: JSON.stringify(queryInfo) },
      });
    },
  },
};
