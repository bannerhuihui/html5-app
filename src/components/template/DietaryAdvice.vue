  ...
  <div>
    ...
    <div class="base-item">
      <div class="base-item-title">
        <van-icon
          ...
          color="#ee0a24"
        />&nbsp;&nbsp;<span>基础代谢</span
        ><span v-if="info" style="color: #ee0a24; font-size: 1.2rem">{{
          info.energyIntake
        }}</span
        >大卡
      </div>
    ...
    <div class="base-item">
      ...
      <div class="base-item-body">
        <div v-if="info">
          <div
            class="three-meals-item"
            v-for="(mealDea, index) in info.mealRatios"
            :key="index"
          >
            <div class="three-meals-item-first">
              <div class="three-meals-item-first-content">
                <span class="three-meals-item-first-span-1">{{
                  getOneDayType(mealDea.mealType)
                }}</span>
                <span class="three-meals-item-first-span-2"
                  >能量占全天{{ mealDea.energyPercentLower }}~{{
                    mealDea.energyPercentUpper
                  }}%</span
                >
              </div>
              <div class="three-meals-item-second">
                {{ mealDea.description }}
              </div>
            </div>
            <div
              v-if="index !== info.mealRatios.length - 1"
              class="three-meals-item-line"
            ></div>
          </div>
        </div>
        <div
        ...
        <div v-if="info">
          <div
            class="like-p"
            v-for="mealDea in info.mealDescriptions"
            :key="mealDea.mealType"
          >
            <div
              style="padding-bottom: 0.4rem; padding-top: 1rem"
              v-if="mealDea.mealType === 'MEAL'"
            >
              {{ mealDea.description }}
            </div>
            <div v-if="mealDea.mealType === 'MEAL_EXTRA'">
              {{ mealDea.description }}
            </div>
          </div>
        </div>
            ...
            <div class="base-chat">
              ...
              <div class="base-chat-ul">
                <div class="base-chat-ul-title">
                  {{ item.foodCategoryName }}&nbsp;&nbsp;{{ item.count
                  }}{{ item.unit }}
                </div>
